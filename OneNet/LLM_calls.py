...
def llm_call(messages, model_name, model=None, tokenizer=None, pipeline=None, do_sample=False, max_new_tokens=1024, model_path=None):
    try:
        if pipeline is not None:
            tokenizer = pipeline[1]
            model = pipeline[0]
        else: tokenizer = AutoTokenizer.from_pretrained(model_path, trust_remote_code=True)
    ...
    if model_name == 'Mistral':
        ...
        try:
            if tokenizer is None: tokenizer = AutoTokenizer.from_pretrained(model_path)
        except Exception as e: print(e)
